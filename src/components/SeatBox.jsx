...
export default function SeatBox({ room, capacity, setSelectedRooms, bookedRooms }) {
    ...
    return (
        <div className={`inline-grid place-items-center ${bookedRooms.length > 0 ? bookedRooms.includes(room) ? 'bg-red-500' : 'bg-grey-all'
            ...
            <p className={`font-Outfit-Bold ${bookedRooms.length > 0 ? bookedRooms.includes(room) && 'bg-red-500' : isChecked ? 'text-white' : 'text-green-save'} cursor-pointer select-none`}>{capacity}</p>
        </div >
    );
}
